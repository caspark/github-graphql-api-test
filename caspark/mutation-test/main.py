...
if not GITHUB_API_TOKEN:
    raise ValueError(
        "API token not specified; set the GITHUB_API_TOKEN environment variable to your Github API token"
    )
...
def graphql(query_or_mutation_string, variables={}):
    body = json.dumps({"query": query_or_mutation_string, "variables": variables})
    ...
    if r_json.get("errors"):
        raise ValueError(
            "GraphQL request error; error follows:\n%s"
            % pprint.pformat(r_json["errors"])
        )
    return r_json["data"]
...
def main():
    ...
    issue_url = graphql(
        MUTATION_CREATE_ISSUE, {"title": "Test issue: %s" % datetime.datetime.now()}
